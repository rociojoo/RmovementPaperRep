...
*Hey Matt, I need help to improve this plot. Starting with putting don't remember at the end of the plot. Kind of like 'Never' in the previous question. I don't know if this will look good, since it will not be filled with gray equally here. But we should try.*

*Also, how can I enumerate the figures and cite these numbers in the text?*
...
*I need help with the same thing here, and to sort the packages ordered by the percentage of excellent (just to try how it looks)*.

If you want to check the numbers for your favorite package, the complete table is [here](make another page for just the table)
*(Matt, could we make a link to this table instead of publishing it directly here?)*
...
If you want to check the numbers for your favorite package, the complete table is [here](hyperlink) *(Matt, could we make a link to this table instead of publishing it directly here?)*
...
```{r, eval=TRUE,echo=FALSE,message=FALSE,warning=FALSE,fig.width=16, fig.height=16}
data_question <- data_all[,grep("q1", colnames(data_all))]
colnames(data_question) <- t(packages)
# I'm dropping trajr that was added in the end and only got 1 response
data_question <- data_question[,-grep("trajr",colnames(data_question))]
...
*Matt, please help making the graph prettier. All of them, actually*
